module fv_restart_mod
  ...
  use fms_mod,    only: file_exist, error_mesg,  lowercase, FATAL, &
       NOTE, read_data, uppercase
  use fms_io_mod,   only: open_restart_file, close_file, write_data
...
! output format
    if (trim(uppercase(oform)) == 'NATIVE') then
        ...
        restart_name = trim(dflnm)
    else if (trim(uppercase(oform)) == 'NETCDF') then
        use_native_format = .false.
        if (trim(dflnm(LEN_TRIM(dflnm):LEN_TRIM(dflnm)-2)) /= ".nc" ) &
             restart_name = trim(dflnm)//".nc"
        if (file_exist(dflnm) .and. .not. file_exist(trim(dflnm)//".nc")) then
    ...
    if (.not. file_exist(dflnm) .and. .not. file_exist(trim(dflnm)//".nc")) &
         call error_mesg ('read_fv_rst', &
...
!!$         call mpp_open (iuic, dflnm, action=MPP_RDONLY, form=MPP_NATIVE, access=MPP_SEQUENTIAL)
                iuic = open_restart_file(dflnm, 'read')
                read(iuic) im, jm, km, nqrst
            endif
        else
            if (file_exist(trim(dflnm)//'.nc')) then
                call mpp_open(iuic, dflnm, &
...
!-----------------------------------------
    ...
    if (do_fms_tracer_manager .and. nq > nt_phys) then !{
        ...
        if( .not. file_exist(fms_tracers_restart_file//'.nc') ) then !{

            if(master) then
                write(*,*)'---- Warning -------'
                write(*,*)'do_fms_tracer_manager=',do_fms_tracer_manager, &
                     ' no of tracers nq=', nq
                write(*,*)'but netcdf restart file ', fms_tracers_restart_file//'.nc', ' does not exist'
                write(*,*)'--------------------'               
            endif
            ...
            if(master) then
                ...
                write(*,*)'These will be read from netcdf file ',fms_tracers_restart_file
            endif

            call mpp_open(iuic, fms_tracers_restart_file, &
                 action=MPP_RDONLY, form=MPP_NETCDF, threading=MPP_SINGLE )
...
!                     call mpp_read(iuic, tracer_fields(jn), w3d)
                      call read_data( fms_tracers_restart_file, tname, &
...
!---------------------------------------------------
    ...
    restart_name ='INPUT/fv_srf_wnd.res'

    if (file_exist(restart_name) .or. file_exist(trim(restart_name)//'.nc')) then
        if (use_native_format) then 
            if( master ) then
!!$         call mpp_open (iuic, 'INPUT/fv_srf_wnd.res', action=MPP_RDONLY,      &
!!$                        form=MPP_NATIVE, access=MPP_SEQUENTIAL)
                iuic = open_restart_file('INPUT/fv_srf_wnd.res', 'read')
                read(iuic) im, jm
...
!            select case (attributes(n)%name)
        ...
        if(master) then
            write(*,*) ' '
            write(*,*) '--------------------------------------------'
            write(*,*) 'File not found: INPUT/fv_srf_wnd.res'
...
! Start from a "dry & cold" isothermal atmosphere at rest
        ...
        if (file_exist('INPUT/surf.res')) then
            mountain = .true.
            if(master) call mpp_open(iuhs, 'INPUT/surf.res', action=MPP_RDONLY,        &
                 form=MPP_NATIVE, access=MPP_SEQUENTIAL)
#ifdef SPMD
            if(master) then
...
!---------------------------
        call read_fv_rst(im, jm, km, ncnst, 'INPUT/fv_rst.res', days, seconds, restart_format)
...
! $Id: fv_restart.F90,v 13.0 2006/03/28 21:06:41 fms Exp $
...
!!$            
!!$         endif
            iuic = open_restart_file(dflnm, 'write')
...
!--------------------------------------------------------------
        ...
        if ( master ) then
            iuic = open_restart_file('RESTART/fv_srf_wnd.res', 'write')
...
! write axes
        ...
        restart_name ='RESTART/fv_srf_wnd.res'
        call mpp_open(iuic, restart_name, &
             action=MPP_OVERWR, form=MPP_NETCDF, threading=MPP_SINGLE )
...
phis_alloc(ids:ide,jcs:jce) = ps_bp(ids:ide,jcs:jce)
        call mpp_write(iuic, phis_field, fv_domain, phis_alloc)
!        call mpp_write(iuic, phis_field, fv_domain, ps_bp)
